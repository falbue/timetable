...
def sync_timetable_to_calendar(data, CALENDAR_ID, colors=None):
    ...
    for week_id, days in data.items():
        ...
        for day_name, lessons in days.items():
            ...
            event_date = (current_monday + timedelta(days=day_index + week_offset)).date()
    ...
    for key in keys_to_delete:
        ...
        delete_batch.add(service.events().delete(
            calendarId=CALENDAR_ID, 
            eventId=event_id
        ))
        deleted_count += 1
...
if __name__ == '__main__':
    ...
    for email in users.keys():
        try:
            ...
            if timetable_data:
                sync_timetable_to_calendar(timetable_data, email, users[email].get("colors"))
